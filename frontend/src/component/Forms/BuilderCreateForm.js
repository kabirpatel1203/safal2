...
const BuilderCreateForm = ({ modalHandler, setIsOpen, parentCallback }) => {
  let initialState = {
    name: "",
    email: "",
    mobileno: "",
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const response = await axios.post("/api/v1/builder/create", data, { headers: { "Content-Type": "application/json" }, withCredentials: true });
      parentCallback(true);
      setIsOpen(false);
    ...
    <div className={Styles.container}>
      ...
      <div className={Styles.personalDetails}>
        ...
        <div className={Styles.personalDetails1}>
          ...
          <input className={Styles.inputTag} onChange={(e) => { formHandler(e) }} value={formData.name} name="name" placeholder='Builder Name' />
