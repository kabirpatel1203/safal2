    ...
    const submitHandler = async (e) => {
        ...
        try {
            if (user.role === "admin" && selectedSalesPerson && selectedSalesPerson !== (data.createdBy && data.createdBy._id ? data.createdBy._id : null)) {
                await axios.put('/api/v1/oem/change-salesperson', { oemId: id, newSalesPersonId: selectedSalesPerson }, { withCredentials: true });
            }
            const response = await axios.put(`/api/v1/oem/update/${id}`, payload, { headers: { "Content-Type": "application/json" }, withCredentials: true });
        ...
        <div className={Styles.container}>
            ...
            <div className={Styles.personalDetails}>
                ...
                <div className={Styles.personalDetails1}>
                    ...
                    <input className={Styles.inputTag} onChange={(e) => { formHandler(e) }} defaultValue={formData.name} value={formData.name} name="name" placeholder='OEM Name' disabled={user.role !== "admin"} />
